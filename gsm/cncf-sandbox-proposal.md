...
- **Primary contact:** TBD
- **Maintainers file:** to be added at contribution time (see [Charter §4]({% link gsm/charter.md %})).
...
Proposed governance is documented in the [Charter]({% link gsm/charter.md %}): maintainer roles, lazy-consensus default with supermajority for normative change, a written change-proposal process, and semantic versioning. A `MAINTAINERS` file would be established at contribution time. Current steward: Poesis.
...
- [Specification]({% link gsm/specification.md %}) — the normative standard.
- [Primer]({% link gsm/primer.md %}) — non-normative introduction.
- [Conformance]({% link gsm/conformance.md %}) — testable requirement catalog.
- [Charter]({% link gsm/charter.md %}) — proposed project governance.
