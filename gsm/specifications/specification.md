...
The **Generative System Model (GSM)** is a vendor-neutral specification for **defining and governing** software-intensive systems, developed as a candidate open standard. Where build pipelines standardize how systems are *built* and OpenTelemetry standardizes how systems are *observed*, GSM standardizes the **THINK** layer: how the *definition* of what a system must be — its purpose, its constraints, its obligations — is expressed, typed, versioned, and made machine-evaluable.
