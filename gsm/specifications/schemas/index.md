---
layout: default
title: GSM Base Archetype Schemas
...
> (the GSM base layer). The `gsm-ontology` repository declares its **own**
...
> gsm-ontology README for that rule.
...
`gsm-ontology` repository — its files follow its own naming convention noted
...
**Extensible** schemas declare no top-level closure. Closure is applied by DM at Ascription authoring time, on the resolved typing Archetype — see GSM §5 ("Statement closure") in [`../model/gsm.puml`](../model/gsm.puml). A base carrying `unevaluatedProperties: false` would reject every property contributed by its extensions, because JSON Schema annotations never propagate from an extending schema into the schema it extends.
...
| `allOf`   | **What facets I include** — applicable property sets; additional cross-cutting concerns composed into the schema | `"allOf": [{"$ref": "gsmarc://gsm-ontology/.../SecurityProperties/v1"}]`   |
...
**GSM base archetype** — One of the 8 JSON Schema files defined by this specification (vendored by conforming processors such as the SIE Definition Manager), corresponding to the 8 GSM classes: Structure, Mechanism, Effector, Receptor, Interaction, Directive, Norm, and the Archetype meta-schema. 7 of 8 are extensible (via `$ref`); the Archetype meta-schema is sealed.
